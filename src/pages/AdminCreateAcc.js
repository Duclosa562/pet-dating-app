...
function AdminCreateAcc() {
    ...
    const createAcc = async () => {
        const createNewAdmin = {usr, fname, lname, pw, email, type};
        const response = await fetch(`/createAdmin`, {
            method: 'POST',
            body : JSON.stringify(createNewAdmin),
            headers: {
                'Content-Type': 'application/json',
            },
        });
        if (response.status === 200){
            alert("Successfully Created a new Admin Account");
        }
        else{
            alert(`Failed to Create a New Admin Account, status code = ${response.status}`);
        }
